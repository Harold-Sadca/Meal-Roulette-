...
function Profile() {
  ...
  function openRecipe(e, recipeType) {
    ...
    if (recipeType == 'drink') {
      const userDrink = currentUser.drinkFavourites.filter(
        (el) => el._id == id
      );
  ...
  if (!loadPage) {
    ...
    return (
      <div key={'profile'} className='profile-container'>
        {authenticated ? (
          <>
            <span key={currentUser.username} className='profile-label'>
          ...
          <>
            <span>
              COME ON MAN YOU CANNOT HAVE A PROFILE WITHOUT LOGGING IN
            </span>
          </>
        )}
      </div>
